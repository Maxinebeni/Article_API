...
def generate_summary(text, is_pdf=False):
    parser = PlaintextParser.from_string(text, Tokenizer("english"))
    summarizer = LexRankSummarizer()
    summary_sentences = summarizer(parser.document, sentences_count=6)  # Adjust the number of sentences for summary
    
    # Join the sentences into a single string with spacing between letters (if it's from a PDF)
    summary_text = ''
    if is_pdf:
        for sentence in summary_sentences:
            summary_text += ' '.join([char + ' ' for char in str(sentence)])
    else:
        for sentence in summary_sentences:
...
def classify():
    ...
    if option == 'url':
        ...
        summary = generate_summary(text, is_pdf=False)
        ...
        summary = generate_summary(text, is_pdf=True)
